  ...
  function openPopup( message, buttonLabel, link, image){
    popup.querySelector('p').innerText=message;
    popup.querySelector('button').innerText=buttonLabel;
    if(link!="")popup.querySelector('button').onclick=()=>{window.open(link)};
  ...
  window.onload = function() {
    ...
    if (isInStandaloneMode()) {
      if(localStorage.getItem("firstTimeWhatsappPopup")==null){
      openPopup("CI SONO NOVITÀ! \n Da oggi è disponibile il canale Whatsapp per ricevere alert in tempo reale \n Entra subito e attiva la campanella.",
        "ENTRA",
        "https://whatsapp.com/channel/0029VbAsWEM1CYoa5ckfFz3G",
        "resources/whatsappAlertIcon.png"
      )
      localStorage.setItem("firstTimeWhatsappPopup", "false");
    }
    } else {
...
//nuovo selettore linea
document.addEventListener("DOMContentLoaded", function () {
  const navItems = document.querySelectorAll('.nav-item');
  const sectionHighlighter = document.querySelector('.sectionHigliterLine');

  // ⬅️ Contenitore stabile di riferimento
  const container = document.querySelector('.containerSezioniNavbar');

  const updateLine = (target) => {
    const left = target.offsetLeft;
    const width = target.offsetWidth;

    sectionHighlighter.style.transform = `translateX(${left}px)`;
    sectionHighlighter.style.width = `${width}px`;
  };

  // Inizializzazione
  updateLine(navItems[0]);
  ...
  window.addEventListener('resize', () => {
    ...
    if (active) updateLine(active);
  });
});

//fine nuovo selettore linea
...
let terremotiRilevanti=0;
  ...
  let start = 0;
    ...
    function chiudiMappaSpecifica(){
      document.querySelector('#specificMapOverlayContainer').style.animation = "slideOutRight 0.2s forwards";
      // Aspetta 2 secondi dopo l'animazione
      setTimeout(function() {
        aperto=false;
      document.querySelector('#specificMapOverlayContainer').style.display='none';
      }, 500);
    }

    function hideShowMap(id){
      const specificMapOverlayContainer = document.getElementById("specificMapOverlayContainer");
      const specificMapOverlay = document.getElementById("specificMapOverlay");
      const earthquakeDiv = document.getElementById(id);
      const earthquakeId = earthquakeDiv.querySelector('#idTerremoto').innerHTML;
      const basePath = window.location.origin + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/"));
      const link = `${basePath}/specificVisualizer.html?eventId=${earthquakeId}&external=0&mapType=`+localStorage.getItem("mapTheme");
      specificMapOverlay.src = link;
      specificMapOverlayContainer.style.animation = "none";
      specificMapOverlayContainer.style.display = "flex";
      specificMapOverlayContainer.style.animation = "slideInLeft 0.2s forwards";
    }

    function hideShowMapSaved(id){
      const specificMapOverlayContainer = document.getElementById("specificMapOverlayContainer");
      const specificMapOverlay = document.getElementById("specificMapOverlay");
      const basePath = window.location.origin + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/"));
      const link = `${basePath}/specificVisualizer.html?eventId=${id}&external=0&mapType=`+localStorage.getItem("mapTheme");
      specificMapOverlay.src = link;
      specificMapOverlayContainer.style.animation = "none";
      specificMapOverlayContainer.style.display = "flex";
      specificMapOverlayContainer.style.animation = "slideInLeft 0.2s forwards";
...
*/
    ...
    function fetchEarthquakes(mag) {
      console.log("Fetch con magnitudo minima: "+mag);
      ...
      */
      ...
      const url = `https://webservices.ingv.it/fdsnws/event/1/query?limit=${listLimit}&orderby=time&format=xml&starttime=${startTime}&endtime=${endTime}&minmagnitude=${mag}`;
...
function importSavedQuakes() {
  ...
  document.body.appendChild(fileInput);
  fileInput.click(); // Simula il click per aprire il selettore di file
  document.body.removeChild(fileInput); // Rimuovi l'input dopo il click
}

function exportConfiguration(){
    const config = {
      savedQuakes: localStorage.getItem('savedQuakes'),
      minMagnitudeFilter: localStorage.getItem('minMagnitudeFilter'),
      dimensioniSegnaposto: localStorage.getItem('dimensioniSegnaposto'),
  ...
  function importConfiguration() {
    const fileInput = document.createElement('input');
    fileInput.type = 'file';
    fileInput.accept = '.json'; // Specifica il formato accettato
    fileInput.onchange = async (event) => {
      const file = event.target.files[0];
      if (!file) {
        console.log("Errore: nessun file selezionato.");
      ...
      try {
        ...
        console.log("Configurazione parsata dal file JSON:", config);
        if (typeof config !== 'object' || config === null) {
          console.log("Errore: il file non contiene un oggetto di configurazione valido.");
          sendNotification("Errore, formato file non valido.", 3000, true);
          return;
        }
        // Aggiorna le impostazioni locali
        localStorage.setItem('savedQuakes', config.savedQuakes || '[]');
        localStorage.setItem('minMagnitudeFilter', config.minMagnitudeFilter || '0');
        localStorage.setItem('dimensioniSegnaposto', config.dimensioniSegnaposto || 'p');
        localStorage.setItem('mapTheme', config.mapTheme || 'light');
        localStorage.setItem('appTheme', config.appTheme || 'black');
        localStorage.setItem('listLimit', config.listLimit || '50');
        // Ricarica i terremoti salvati
        window.location.reload(); // Ricarica la pagina per applicare le nuove impostazioni
        console.log("Configurazione importata con successo:", config);
        sendNotification("Configurazione importata con successo.", 3000, false);
      } catch (err) {
        console.error("Errore durante l'importazione della configurazione:", err);
        sendNotification("Errore durante l'importazione della configurazione.", 3000, true);
      }
  };
  document.body.appendChild(fileInput);
  fileInput.click(); // Simula il click per aprire il selettore di file
...
//funzione per eliminare i dati dalvati
function deleteSavedData(){
  localStorage.clear();
  console.log("Tutti i dati salvati sono stati eliminati.");
  sendNotification("Dati salvati eliminati con successo.", 3000, false);
  caricaTerremotiSalvati();
  closeDialog(); // Aggiungi questa funzione per chiudere il dialog
  window.location.reload(); // Ricarica la pagina per aggiornare lo stato
}

function dialog(message, callbackFunction) {
  document.querySelector('#dialogMessage').textContent = message;
  document.querySelector('#dialogOverlay').style.display = 'flex';
  document.querySelector('#dialogOverlay').style.animation = 'fadeIn 0.3s forwards';
  
  // Assegna la funzione direttamente all'evento click
  document.querySelector('#dialogConfirmButton').onclick = function() {
  ...
  // Assegna la funzione direttamente all'evento click
  document.querySelector('#dialogCancelButton').onclick = function() {
    closeDialog();
  };
}
...
function changeTheme() {
  ...
  const theme = document.getElementById("appThemeSelector").value;
  console.log("Cambiato tema in " + theme);
  localStorage.setItem("appTheme", theme);

  const allowedThemes = ['glass', 'black', 'pink', 'blue', 'brown', 'pride', 'xMAS'];

  const lightThemedElements = document.querySelectorAll('.lightThemed');
  const darkThemedElements = document.querySelectorAll('.darkThemed');

  // Utility: rimuove tutte le classi tema specifiche
  const clearThemeClasses = (elements, suffix) => {
    elements.forEach(el => {
      allowedThemes.forEach(t => el.classList.remove(`${t}Theme${suffix}`));
    });
  };

  // Utility: aggiunge il nuovo tema selezionato
  const applyThemeClass = (elements, suffix) => {
    elements.forEach(el => el.classList.add(`${theme}Theme${suffix}`));
  };

  clearThemeClasses(lightThemedElements, 'Light');
  applyThemeClass(lightThemedElements, 'Light');

  clearThemeClasses(darkThemedElements, 'Dark');
  applyThemeClass(darkThemedElements, 'Dark');

  // Gestione personalizzata per il tema glass (senza toccare classi del body!)
  if (theme === 'glass') {
    ...
    headerbar.classList.add('glassyHeaderbarAdjustments');
    navbar.classList.add('glassyNavbarAdjustments');
    document.getElementById("selettoreLinea").style.display = "none"; // Nascondi il selettore della linea
    // Protezione: controlla che esistano
    if (body && lista) {
      lista.classList.remove('glassThemeDark');
      body.style.backgroundColor = 'rgba(20,20,20)';
      body.style.backgroundImage = 'url("resources/texture/dark_glass.png")';
      body.style.backdropFilter = 'none'; // eventuale reset
    }
  } else {
    // Reset dello sfondo se si cambia da glass a qualcos'altro
    const body = document.getElementById("body");
    const headerbar = document.querySelector("#headerbar");
    const navbar = document.querySelector("#navbar");
    headerbar.classList.remove('glassyHeaderbarAdjustments');
    navbar.classList.remove('glassyNavbarAdjustments');
    headerbar.classList.add('headerbar');
    navbar.classList.add('navbar');
    document.getElementById("selettoreLinea").style.display = "block"; // Mostra il selettore della linea
    if (body) {
